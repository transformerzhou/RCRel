...
class GATLayer(nn.Module):
    def __init__(self, num_rels):
        ...
        self.enc_self_attn = MultiHeadAttention(num_rels=num_rels)
...
class GAT_module(nn.Module):
    def __init__(self, n_layers=1, num_rels=24):
        super(GAT_module, self).__init__()
        self.layers = nn.ModuleList([GATLayer(num_rels) for _ in range(n_layers)])
...
class MultiHeadAttention(nn.Module):
    def __init__(self, d_model=768, n_heads=12, num_rels=24):
